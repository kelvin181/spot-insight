...
def get_top_tracks(time_range, limit = 30):
    ...
    response = requests.get(f"https://api.spotify.com/v1/me/top/tracks?time_range={time_range}_term&limit={limit}", headers=headers)
    if not response:
...
def get_playlists():
    ...
    while True:
        response = requests.get(f"https://api.spotify.com/v1/me/playlists?limit=50&offset={offset}", headers=headers)
        if not response:
            return None
        json_result = json.loads(response.content)["items"]
...
def create_playlist_song_dict(json_result):
    items = []
    for item in json_result:
        album_image = item["track"]["album"]["images"][0]["url"] if item["track"]["album"]["images"] else url_for('static', filename='no_image.png')
        link = item["track"]['external_urls']['spotify'] if item["track"]['external_urls'] else None
